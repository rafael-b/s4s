...
# OBJECT
  ...
  def self.id_link(item,options={})
    ...
    return link_to(options[:link_text],"/#{item.class.name.tableize}/#{item.id}")
  end
...
# BOOLEAN
  ...
  def self.yn_with_style(value,options={})
    options[:yes_class] ||= 'boolean_flag yes_flag'
    options[:no_class]  ||= 'boolean_flag no_flag'
    options[:yes]       ||= "<span class='#{options[:yes_class]}'>YES</span>"
    options[:no]        ||= "<span class='#{options[:no_class]}'>NO</span>"
