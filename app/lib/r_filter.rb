...
# SORT ORDER
  ...
  def set_secondary_sort(s)
    self.secondary_sort = s.strip
    self.sql_sortorder  = "#{self.sort_by} #{self.sort_by_order}" if !self.sort_by.blank? && !self.sort_by_order.blank?
    self.sql_sortorder += ", #{self.secondary_sort}" unless self.secondary_sort.blank?
  end
