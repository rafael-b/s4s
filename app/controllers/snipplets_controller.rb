class SnippletsController < ApplicationController
#--------------------------------------------------------------------------------------------------------------------------------
...
# generate edit_x and update_x pairs for various fields
  ...
  [:content,:scripts,:styles].each do |field|

    define_method("edit_#{field}") do
      @object = Snipplet.find(params[:id])
    end
    ...
    define_method("update_#{field}") do
      ...
      if @object.update_attributes(params[model_sym])
        ...
        redirect_to("/snipplets/#{@object.id}#{show_what}", :notice=>"Object's #{field} updated.")
      else
