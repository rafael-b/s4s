...
#--------------------------------------------------------------------------------------------------------------------------------
  ...
  def filter_settings
    ...
    @filter.register_dropdowns('lecture_id')
...
# generate edit_x and update_x pairs for various fields
  ...
  [:meta,:abstract,:content,:questions].each do |field|

    define_method("edit_#{field}") do
      @object = Quiz.find(params[:id])
    end
    ...
    define_method("update_#{field}") do
      ...
      if @object.update_attributes(params[model_sym])
        ...
        redirect_to("/quizzes/#{@object.id}#{show_what}", :notice=>"Object's #{field} updated.")
      else
